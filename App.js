...
const App = () => {
     ...
     messaging().requestPermission().then(authStatus => {
       ...
       if (authStatus === messaging.AuthorizationStatus.AUTHORIZED || authStatus === messaging.AuthorizationStatus.PROVISIONAL) {
         messaging().getToken().then(token => {
           ...
           axios.put("https://restaurant-portal.foodtruck-uat.com/cloud-message/replace", { new_message_token: token }).catch(err => {
             console.info(err)
           })
         }).catch(err => {
   ...
   return (
     <SafeAreaView style={backgroundStyle}>
       ...
       <WebView style={{ flex: 1 }} source={{uri: "https://restaurant-portal.foodtruck-qa.com/"}} />
     </SafeAreaView>
   );
 };
